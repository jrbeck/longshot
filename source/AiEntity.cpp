		...
		for (int i = 0; i < AI_INVENTORY_SIZE; i++) {
			if (mInventory[i] > 0) {
				...
				if (item.type == ITEMTYPE_GUN_ONE_HANDED && r_numi (0, 10) < 3) {
					...
					size_t itemPhysicsHandle =
						physics.createEntity (OBJTYPE_ITEM, mWorldPosition, itemForce, time, true);

					physics.setItemHandleAndType (itemPhysicsHandle, mInventory[i],
						itemManager.getItemType (mInventory[i]));
				}
...
//		updateState(time, worldMap, physics, aiEntities, itemManager);
	...
	if (mTargetPhysicsEntity == NULL) {
		...
		if (mTargetPhysicsEntity->type != OBJTYPE_TIGER_BAIT &&
			worldMap.lineOfSight (mWorldPosition, targetPosition))
		{
			if (mInventory[0] > 0 &&
				itemManager.getItem (mInventory[0]).type == ITEMTYPE_GUN_ONE_HANDED)
			{
	...
	ItemManager &itemManager)
{
	...
	for (size_t i = 0; i < totalStates && !isDone; i++) {
		if (spec->stateMachine.mStates[i].state == mCurrentState) {
			if (testCondition(
				spec->stateMachine.mStates[i].condition,
				time,
				worldMap,
				physics,
				aiEntities,
...
//		printf ("message: to: %d, from: %d\n", message.recipient, message.sender);
		...
		case PHYS_MESSAGE_DAMAGE:
			if (message.sender != mPhysicsHandle &&
				message.sender != MAILBOX_PHYSICS) {

				if (physics.getIndexFromHandle(message.sender) >= 0) {
					if (physics.getEntityByHandle(message.sender)->aiType == AITYPE_PLAYER &&
						mType == PLAYER_SPECIES)
					{
	...
	ItemManager &itemManager)
{
	...
	if (physics.getIndexFromHandle(mTargetPhysicsHandle) >= 0) {
		...
		{
			desiredHeight = targetPosition.y + 14.0 + (2.0 * sin (time));
		}
		else {
			// this is totally random...
			desiredHeight = 38.0 + (2.0 * sin (time));
		}
	...
	ItemManager &itemManager)
{
	...
	if (mTargetPhysicsHandle == 0 &&
		mPhysicsEntity->worldViscosity > 0.0)
	{
...
void AiEntity::updateTarget (double time, WorldMap &worldMap, physics_c &physics, vector<AiEntity *> &aiEntities) {
	...
	if (mTargetPhysicsHandle != 0) {
		...
		if (mTargetPhysicsEntity == NULL) {
			...
			if (mTargetPhysicsEntity->health <= 0.0 &&
				mTargetPhysicsEntity->type == OBJTYPE_PLAYER)
			{
				mTargetPhysicsHandle = 0;
				mTargetPhysicsEntity = NULL;
			}
