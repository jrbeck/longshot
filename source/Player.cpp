...
void player_c::updateTargetBlock (WorldMap &worldMap) {
	...
	if (worldMap.rayCastSolidBlock(eye, finalTarget, mTargetBlock, mTargetBlockFace)) {
...
void player_c::updateHud (ItemManager &itemManager) {
	...
	mHud.addText (v2d_v (0.4, 0.0), v2d_v (0.2, 0.05), fontSize,
		healthString, TEXT_JUSTIFICATION_CENTER, color, NULL);

	// display the primary equip
	if (mInventory.mPrimaryItem != 0) {
		item_t currentEquip = itemManager.getItem (mInventory.mPrimaryItem);
		sprintf (text, "%s\0", currentEquip.name);
		mHud.addText (v2d_v (0.0, 0.0), v2d_v (0.2, 0.05), fontSize,
			text, TEXT_JUSTIFICATION_LEFT, color, NULL);

		// display the ammo
		switch (currentEquip.type) {
			case ITEMTYPE_GUN_ONE_HANDED:
				sprintf (text, "ammo: %d\0", mInventory.mAmmoCounter[currentEquip.ammoType]);
				mHud.addText (v2d_v (0.0, 0.05), v2d_v (0.2, 0.05), fontSize,
					text, TEXT_JUSTIFICATION_LEFT, color, NULL);
				break;
		}
	}
	...
	if (mInventory.mSecondaryItem != 0) {
		item_t currentEquip = itemManager.getItem (mInventory.mSecondaryItem);
		sprintf (text, "%s\0", currentEquip.name);
	...
	color[3] = 0.75f;
	mHud.addText (v2d_v (0.495, 0.5), v2d_v (0.01, 0.01), fontSize,
		crossHair, TEXT_JUSTIFICATION_CENTER, color, NULL);
...
//		worldMap.setShowWorldChunkBoxes (mShowWorldChunkBoxes);
	...
	block_t *block = worldMap.getBlock(v3d_add (mFinalHeadOffset, mPos));
	int blockType;
	if (block == NULL) {
...
*/
	...
	if (mMeleeStatePrimary.hasUsed) {
		if (mIsBlockTargeted) {
			worldMap.clearBlock(mTargetBlock);
		}
...
void player_c::placeLight(LightManager &lightManager, WorldMap &worldMap, GameInput &gi) {
	if (mPlacedBlock) {
		...
		if (gi.isClickMouse1() && mIsBlockTargeted) {
			...
			lightManager.addLight(pos, 10.0, color, worldMap);
		}
	}
}
