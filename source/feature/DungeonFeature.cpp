	...
	World& world)
{
	...
	addLights( world, dungeon, cornerIndex, dungeonSide, height - 3 );
...
void DungeonFeature::hollowOutDungeon2x( DungeonUtil* dungeon, WorldMap& worldMap, const v2di_t& cornerIndex, int dungeonSide, int height ) {
	int worldX = cornerIndex.x * WORLD_CHUNK_SIDE;
	int worldZ = cornerIndex.y * WORLD_CHUNK_SIDE;
		...
		for( int dx = 0; dx < dungeonSide; dx++ ) {
			...
			if (type != DUNGEON_TILE_WALL) {
				...
				if( type == DUNGEON_TILE_WATER ) {
					draw2x2Block( worldPos, BLOCK_TYPE_WATER, worldMap );
				}
				else {
...
void DungeonFeature::clearDungeonBlock (int worldX, int worldY, int worldZ, WorldMap &worldMap) {
	v3di_t nearCorner = {
		worldX,
		worldY,
		worldZ
...
void DungeonFeature::draw2x2Block( v3di_t worldPos, char blockType, WorldMap& worldMap ) {
...
void DungeonFeature::addLights( World& world, DungeonUtil* dungeon, const v2di_t& cornerIndex, int dungeonSide, int ceilingHeight ) {
	LightManager* lightManager = world.getLightManager();
	WorldMap& worldMap = *world.getWorldMap();

	IntColor lightColor = { LIGHT_LEVEL_MAX, LIGHT_LEVEL_MAX, LIGHT_LEVEL_MAX };
	...
	for( int dz = 0; dz < dungeonSide; dz += 5 ) {
		...
		for( int dx = 0; dx < dungeonSide; dx += 5 ) {
			...
			if (type != DUNGEON_TILE_WALL) {
				worldPos.y = ceilingHeight;
				lightManager->addLight( lightPos, 7.5, lightColor, worldMap );
			}
		}
	}
