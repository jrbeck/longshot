...
//	int zIndex = worldPosition.z / WORLD_CHUNK_SIDE;
	...
	int xIndex = static_cast<int>(floor (static_cast<double>(worldPosition.x) /
		static_cast<double>(WORLD_CHUNK_SIDE)));
	int zIndex = static_cast<int>(floor (static_cast<double>(worldPosition.z) /
		static_cast<double>(WORLD_CHUNK_SIDE)));
...
void WorldMap::clearBlock( const v3di_t& position ) {
	...
	// FIXME: the generates rocky outcroppings...no good
	// but we need it to drill into the ground
	...
	for (pos.z = a.z; pos.z <= b.z; pos.z++) {
		for (pos.y = a.y; pos.y <= b.y; pos.y++) {
			for (pos.x = a.x; pos.x <= b.x; pos.x++) {
				...
				if (gBlockData.get(blockType)->solidityType != BLOCK_SOLIDITY_TYPE_ROCK ||
					gBlockData.get(blockType)->alpha < 1.0) {
					clearBlock (pos);
				}
...
int WorldMap::fillSphere( const v3d_t& pos, double radius, int blockType, BYTE uniqueLighting ) {
	...
	start = v3di_v (static_cast<int>(pos.x - (radius + 1)),
		static_cast<int>(pos.y - (radius + 1)),
		static_cast<int>(pos.z - (radius + 1)));

	stop = v3di_v (static_cast<int>(pos.x + (radius + 1)),
		static_cast<int>(pos.y + (radius + 1)),
		static_cast<int>(pos.z + (radius + 1)));
...
static v3d_t faceAdd[6] = {
	...
	for (double t = step; t <= 1.0; t += step) {
		...
		if (block != NULL &&
			gBlockData.get(block->type)->solidityType == BLOCK_SOLIDITY_TYPE_PLANT)
		{
		...
		if (isSolidBlock(blockPosition)) {
			...
			for (int s = 0; s < NUM_BLOCK_SIDES; s++) {
				if (faceHitResults[s]) {
					if (faceHitTimes[s] > 0.0 &&
						faceHitTimes[s] < soonestHit)
					{
