...
//	glEnable (GL_COLOR_MATERIAL);
  ...
  for (size_t index = 0; index < numEntities; index++) {
    ...
    if (physicsEntity->active && physicsEntity->color[3] >= 1.0) {
      // ignores players and AI
      if (type == OBJTYPE_PLAYER || type == OBJTYPE_AI_ENTITY) {
        continue;
      }
      else if (type == OBJTYPE_ITEM || type == OBJTYPE_GRENADE)
      {
        // FIXME: items should be drawn by ItemManagerView
        drawTextured(*physicsEntity, worldMap, assetManager);
      }
      else {
        glDisable(GL_TEXTURE_2D);
          drawEntity(*physicsEntity);
        glEnable(GL_TEXTURE_2D);
...
void PhysicsView::drawTransparentEntities(vector<PhysicsEntity*>* physicsEntities, AssetManager& assetManager, bool inWater) {
  ...
  for (size_t index = 0; index < numEntities; index++) {
    physicsEntity = (*physicsEntities)[index];
    if (physicsEntity->active && (physicsEntity->worldViscosity > 0.0) == inWater && physicsEntity->color[3] < 1.0f) {
      drawEntity(*physicsEntity);
    }
  }
