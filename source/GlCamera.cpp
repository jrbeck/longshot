...
void gl_camera_c::resize_screen (int w, int h) {
	mScreenWidth = w;
	mScreenHeight = h;
...
void gl_camera_c::set_fov_near_far (double fov, double n, double f) {
	fovy = fov;
...
void gl_camera_c::set_near_far_min_max (double nmin, double nmax, double fmin, double fmax) {
	near_min = nmin;
	near_max = nmax;
	far_min = fmin;
	far_max = fmax;
...
void gl_camera_c::adjust_near (double amount) {
	set_fov_near_far (fovy, near_d + amount, far_d);
}



void gl_camera_c::adjust_far (double amount) {
	set_fov_near_far (fovy, near_d, far_d + amount);
}



void gl_camera_c::set_far (double new_far) {
	set_fov_near_far (fovy, near_d, new_far);
}



v2d_t gl_camera_c::getNearAndFar (void) {
	v2d_t nearAndFar = v2d_v (near_d, far_d);
...
void gl_camera_c::setNearAndFar (v2d_t nearAndFar) {
...
v3d_t gl_camera_c::getPosition(void) {
	return mPosition;
}


void gl_camera_c::perspective (void) {
...
void gl_camera_c::look_at(v3d_t position, v3d_t target, v3d_t upVector) {
	mPosition = position;
	mTarget = target;
...
// FIXME: this is very specific to longshot::world::draw sun ()
void gl_camera_c::updateGlVariables(void) {
	perspective ();
...
void gl_camera_c::set_planes (void) {
...
int gl_camera_c::point_visible (v3d_t p) {
	for (int i = 0; i < 6; i++) {
...
int gl_camera_c::point_visible (v3di_t p) {
	v3d_t pi;
...
int gl_camera_c::bounding_box_test (BoundingBox &box) {
/*	v3d_t corner[8];
	bool all_in = true;
	...
	printf ("ERROR: gl_camera_c::bounding_box_test is broken. please fix.\n");
...
int gl_camera_c::bounding_sphere_test (const BoundingSphere &sphere) const {
	bool all_in = true;
